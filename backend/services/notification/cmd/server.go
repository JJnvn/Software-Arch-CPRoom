...
func main() {
	...
	eventsCh, err := rabbitRes.Connection.Channel()
	if err != nil {
		log.Fatalf("RabbitMQ events channel failed: %v", err)
	}
	...
	pushSender := internal.NewLogPushSender()
	...
	eventConsumer := internal.NewDomainEventConsumer(service, eventsCh, rabbitRes.EventsQueue)
